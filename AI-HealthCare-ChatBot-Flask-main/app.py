...
df_tr = pd.read_csv(r'C:\Users\Sravya Appikonda\Downloads\AI-HealthCare-ChatBot-Flask-main\AI-HealthCare-ChatBot-Flask-main\Medical_dataset\Training.csv')
df_tt = pd.read_csv(r"C:\Users\Sravya Appikonda\Downloads\AI-HealthCare-ChatBot-Flask-main\AI-HealthCare-ChatBot-Flask-main\Medical_dataset\Testing.csv")
...
# load model
knn_clf = joblib.load(r"C:\Users\Sravya Appikonda\Downloads\AI-HealthCare-ChatBot-Flask-main\AI-HealthCare-ChatBot-Flask-main\model\knn.pkl")
...
def getDescription():
    ...
    with open(r'C:\Users\Sravya Appikonda\Downloads\AI-HealthCare-ChatBot-Flask-main\AI-HealthCare-ChatBot-Flask-main\Medical_dataset\symptom_Description.csv') as csv_file:
...
def getSeverityDict():
    ...
    with open(r'C:\Users\Sravya Appikonda\Downloads\AI-HealthCare-ChatBot-Flask-main\AI-HealthCare-ChatBot-Flask-main\Medical_dataset\symptom_severity.csv') as csv_file:
...
def getprecautionDict():
    ...
    with open(r'C:\Users\Sravya Appikonda\Downloads\AI-HealthCare-ChatBot-Flask-main\AI-HealthCare-ChatBot-Flask-main\Medical_dataset\symptom_precaution.csv') as csv_file:
...
# Function to write new user data to CSV
import csv
...
# Function to read CSV file and return a dictionary of usernames and passwords
def read_users_from_csv():
    ...
    with open('users.csv', newline='') as csvfile:
        ...
        for row in reader:
            username = row[0]
            password = row[1]
            users[username] = password  # Store password in a dictionary
    return users

# Function to check credentials against the CSV file
def check_credentials(username, password):
    users = read_users_from_csv()  # Read the users from CSV
    if username in users:  # Check if the username exists
        if users[username] == password:  # Check if the password matches
            return True
    return False
...
def register():
    if request.method == 'POST':
        ...
        df_users = pd.read_csv('C:/Users/Sravya Appikonda/Downloads/AI-HealthCare-ChatBot-Flask-main/AI-HealthCare-ChatBot-Flask-main/users.csv', header=None, names=['username', 'password'])

        if username in df_users.values:
...
def get_bot_response():
    ...
    if session['step'] == "Depart":
        session['step'] = "BFS"
        return "Well, Hello " + session["name"] + ", now I will be asking some few questions about your symptoms to see what you should do. Tap Yes to start diagnostic!"
    if session['step'] == "BFS":
        session['step'] = "FS"  # first symp
        return "Can you precise your main symptom from the list " + session["name"] + " ?"
    if session['step'] == "FS":
        sym1 = s
        ...
        if sim1 == 1:
            ...
            return "Do you want to QUIT consultation [yes/no]?"
    if session['step'] == "RS1":
        temp = session['FSY']
        psym1 = temp[2]
        psym1 = psym1[int(s)]
        ...
        if result is not None:
            if result[0] != session["testpred"]:
                ...
                return "as you provide me with few symptoms, I am sorry to announce that I cannot predict your " \
                       "disease for the moment!!! <br> Can you specify more about what you are feeling or Tap quit to " \
            ...
            return "can you specify more what you feel or Tap quit to stop the conversation"
    ...
    if session['step'] == "Severity":
        ...
        if calc_condition(session["all"], int(s)) == 1:
            return "you should take the consultation from doctor <br> Tap quit to exit"
        else:
    ...
    if session['step'] == "BYE":
        ...
        if s.lower() == "yes":
            ...
            return "THANKS " + name + " for using me for more information please contact <b> +91 8688648114 </b>"
